...
def students():
    ...
    cursor.execute("SELECT FullName, Email, Department FROM Users WHERE Role='Student'")
...
def staff():
    ...
    cursor.execute("SELECT FullName, Email FROM Users WHERE Role='Staff'")
...
def approved_reports():
    ...
    cursor.execute("SELECT * FROM Reports WHERE Status='Approved'")
    reports = cursor.fetchall()
...
def rejected_reports():
    ...
    cursor.execute("SELECT * FROM Reports WHERE Status='Rejected'")
    reports = cursor.fetchall()
...
def approve(id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE Reports SET Status='Approved' WHERE ReportID=?", (id,))
    conn.commit()
    conn.close()
    return redirect('/admin-dashboard')
...
def reject(id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE Reports SET Status='Rejected' WHERE ReportID=?", (id,))
    conn.commit()
    conn.close()
    return redirect('/admin-dashboard')
...
def submit_report():
    if 'user_id' in session:
        ...
        cursor.execute("""
            INSERT INTO Reports (UserID, Title, Description, Location, ImagePath, AI_Score)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, title, description, location, filename, ai_score))
